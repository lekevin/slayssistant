...
At 468 chunks, the index is a 1.8 MB float32 file. A flat cosine scan over it takes about a millisecond —
...
```bash
npm test           # retrieval math + citation resolution + question shape
npm run typecheck
```
...
Numbers in `eval/results.md`, regenerated by `npm run eval`. Two results are worth stating plainly.
...
rule already applied to cards — moved dense-only recall@8 from **79.4% to 97.1%**. No retrieval technique
in the ablation moved anything close to that far. The trace panel is what made it visible.

**The naive baseline is hard to beat at this corpus size.** The prototype's keyword+IDF scoring reaches
97.1% recall@8. Dense embeddings alone do not beat it; hybrid fusion matches it; only the cross-encoder
reranker clearly improves on it, and mostly in MRR (0.79 → 0.91) rather than recall, since recall@8
saturates on a set this small. With n=34 the minimum detectable effect is ~24 points, so none of these
gaps are statistically resolved — the honest summary is *directional, not established*.

The golden set needs to be both larger and harder before the table can carry real weight. It was seeded
largely from the rulebook's own FAQ, whose distinctive vocabulary is exactly what keyword search is good
at — a selection bias that flatters the baseline.
